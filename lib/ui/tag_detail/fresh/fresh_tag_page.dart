...
class FreshTagPage extends StatefulWidget {
  final String path;

  const FreshTagPage({Key? key, required this.path}) : super(key: key);

  @override
  State<FreshTagPage> createState() => _FreshTagPageState();
}

class _FreshTagPageState extends State<FreshTagPage> with AutomaticKeepAliveClientMixin {
  ...
  void fetchData() async {
    ...
    final data = await repository.fetchTagDetail(widget.path, 'fresh', null);
  ...
  void loadMore() async {
    if (nextPage != null &&
      ...
      final data = await repository.fetchTagDetail(widget.path, 'fresh', nextPage);
      nextPage = data.nextCursor;
      setState(() {
        posts.addAll(data.posts);
      });
